...
const createClass = () => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
      let data;
      {
          const res = fs_1.readFileSync(`${customTemplatesFolder}/filesclasses.json`);

          const tempdata = JSON.parse(res.toString());
          templates = [];
          for (let tname in tempdata.templates) {
          ...
          for (let file in tempdata.templates[selected]) {
              const value = fs_1.readFileSync(`${customTemplatesFolder}/${selected}/${file}`);
              let data = value.toString().replace(new RegExp('easyclass', 'g'), val);
              fs_1.writeFileSync(`${currentFolder}/${tempdata.templates[selected][file].folder}/${val}.${tempdata.templates[selected][file].extension}`, data);
              vscode.workspace.openTextDocument(`${currentFolder}/${tempdata.templates[selected][file].folder}/${val}.${tempdata.templates[selected][file].extension}`)
